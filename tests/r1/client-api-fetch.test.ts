...
function installMemoryStorage(g: LooseGlobal): Map<string, string> {
...
function cleanupTestGlobal(g: LooseGlobal) {
...
test("clientBearerHeaders：无 token 时为空对象，有 token 时附带 Bearer", () => {
  ...
  const store = installMemoryStorage(g);
...
test("clientApiFetch：默认附带 Bearer 与 credentials include，且不覆盖已有 Authorization", async () => {
  const g = globalThis as unknown as LooseGlobal;
  const store = installMemoryStorage(g);
  store.set(CLIENT_ACCESS_TOKEN_KEY, "session-tok");
