...
async function handlePostViewPOST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (postRow.status !== "published" || postRow.visibility === "private") {
    return NextResponse.json(localizedErrorResponse(request, "post.notAccessible"), { status: 403 });
  }
  ...
  if (!limiter.allowed) {
    return jsonRateLimitError(request, limiter.retryAfterSeconds, "post.viewRateLimit");
  }
  ...
  // 返回成功响应
  return NextResponse.json(localizedSuccessResponse(request, true, "post.viewIncrementSuccess"), {
    status: 200,
  });
}
