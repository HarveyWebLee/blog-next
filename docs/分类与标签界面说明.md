...
前台页面在 **App Router 多语言段** 下，形如：

```
app/[lang]/categories/page.tsx      # 分类列表（公开展示）
app/[lang]/categories/manage/...   # 分类管理子路由（若已启用）
app/[lang]/tags/page.tsx            # 标签列表
app/[lang]/tags/manage/...         # 标签管理子路由
components/ui/tag-cloud.tsx        # 标签云等可复用组件
lib/data/mock-data.ts              # 开发用 Mock（生产数据来自 API）
types/blog.ts
```

> 标签颜色等字段在 **`tags` 表** 与后台表单中维护；历史上文档曾示例 `TagColorPicker` 独立文件，**以 `components/` 目录实际文件为准**。

## 🎨 组件说明

### 1. 分类页面 (`/{lang}/categories`)
...
标签颜色字段存储在数据库 `tags.color`；管理界面可用 HeroUI `Input`（type=`color` 或十六进制文本）等自行封装，无需依赖固定组件名。
...
- 分类页面: `/zh-CN/categories`（或 `en-US`、`ja-JP`）
- 标签页面: `/zh-CN/tags`
...
// 颜色选择若未抽成独立组件，可在管理页表单内用 Input 等实现
```
