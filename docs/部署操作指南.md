...
> **文档导航**：[文档索引.md](./文档索引.md)汇总全库说明。开发环境**清空/重建库表**请优先阅读 [数据库重置与验证指南.md](./数据库重置与验证指南.md)（含 `pnpm db:reset:dev` 等，与 `package.json` 一致）。  
> **环境变量**：以仓库根目录 `env.example` 为准（`DB_*`、`JWT_*` 等）；**不使用** `DATABASE_URL` / NextAuth 变量。

## 📋 概述

本文档详细记录了从零开始部署「荒野博客」的常见过程，包括 MySQL 容器创建、数据库初始化、环境配置、系统启动等。系统基于 Next.js 15、MySQL 与 Drizzle ORM。

**跨平台提示**：下文若使用 `sed -i ''`，为 **macOS/BSD sed** 写法；在 **GNU sed**（常见 Linux 或 Git Bash）可改为 `sed -i` 或 `sed -i.bak`。亦可跳过 sed，直接用编辑器修改 `.env.local`。  
**数据卷路径**：`-v` 挂载请改为你本机目录（示例中出现的 `/Users/...` 仅为历史记录）。
